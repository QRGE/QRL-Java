...
public class DoubleCircularLinkedList<E> extends AbstractList<E> {
    ...
    public void add(int index, E element) {
        ...
        if (index == size){
            ...
            Node<E> newNode = new Node<>(element, first, preNode);      // 创建newNode
            last = newNode;                                             // last指向newNode
            if (preNode == null){                                       // 空链表的情况
                first = newNode;                                        // 将first指向新的节点newNode
                first.next = first;                                     // 只有一个元素, next指向自己
                first.prev = first;                                     // prev同理
            }else {
                preNode.next = newNode;                                 // preNode的next需要指向newNode
                first.prev = newNode;                                   // 头节点的prev需要指向新添加的newNode形成环
            }
        } else {
    ...
    public E remove(int index) {
        ...
        if (size == 1){
            ...
            prevNode.next = nextNode;             // prevNode的next指向nextNode,删除了deletedNode
            nextNode.prev = prevNode;             // 同上
            if (index == 0){                      // 删除头节点的情况
                first = nextNode;                 // nextNode变成了新的first
            }
            if (index == size - 1){               // 删除尾节点的情况
                last = prevNode;                  // preNode成为的新的last节点
            }
            deletedElement = deletedNode.element; // 获取被删除节点的element, 用于返回
            size--;                               // 删除数据一个数据后size-1
        }
        return deletedElement;                    // 返回被删除的节点的数据element
    }
